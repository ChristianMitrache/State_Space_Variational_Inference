...
def sample_from_Poisson_LDS(epsilon_precision: torch.Tensor, LDS_matrix: torch.Tensor,
    ...
    sampled_epsilon = MultivariateNormal(loc=torch.zeros(epsilon_precision.shape[0]),
                                         precision_matrix=epsilon_precision).sample(
        (time_steps,))
    ...
    sample_tensor[0, :] = torch.poisson(torch.exp(z_curr @ loading_matrix.T + loading_bias))

    # Iterative sampling assuming z_t = Az_{t-1} + epsilon_t
    for i in range(1, time_steps):
        z_curr = LDS_matrix @ z_curr + sampled_epsilon[i, :]
        sample_tensor[i, :] = torch.poisson(torch.exp(z_curr @ loading_matrix.T + loading_bias))
    ...
    def forward(self, x: torch.Tensor, z: torch.Tensor, batched=True):
        ...
        if batched:
            return torch.mean(
                ...
                torch.sum(Poisson(rate=torch.exp(self.loading_transformation(z[:, 1:, :]))).log_prob(x[1:, :]),
                          dim=(1, 2)))
        else:
            return torch.mean(
                MultivariateNormal(loc=torch.zeros(z.shape[2]), precision_matrix=self.epsilon_precision).log_prob(
                    z[:, 0, :]) + \
                torch.sum(MultivariateNormal(loc=self.LDS_transformation(z[:, :-1, :]),
                                             precision_matrix=self.epsilon_precision).log_prob(z[:, 1:, :]), dim=1) + \
