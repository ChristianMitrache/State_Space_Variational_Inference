...
class Approximate_Inference_Loss(Module):
    ...
    def forward(self,x,n):
        ...
        print("Approximate Posterior Loss: " +str(self.Approx_Post(x)))
        print("User Loss: " + str(self.User_model(x,self.Approx_Post.Sample(n, x))))
...
def Create_Time_Series_Data_Loader(X:torch.Tensor,batch_size:int,num_workers = 0,
    ...
    batched_time_sampler = BatchSampler(SequentialSampler(Tensor_data), batch_size=batch_size, drop_last=True)
...
def train_model_epoch(loader,loss_fn,optimizer,scheduler,
  ...
  for batch_index, data in enumerate(loader):
      ...
      # Forward pass:
      optimizer.zero_grad() #<- just in case extra gradients are left over
...
def print_computational_graph(network):
    """
    This function prints the computational graph generated by torch after
    :param network: torch instance (whether that's )
    :return: None
    """
    current_function = network.grad_fn
    print(current_function)
    while current_function.next_functions[0][0]:
        print(current_function.next_functions[0][0])
        current_function = current_function.next_functions[0][0]